...
def stitch(unit_id, slide, fovs, dest, out_dir, P):
    ...
    _ADAPT_TARGET = True   # hi-res: set target_long ~1um/px from micron span (extent unchanged)
    ...
    if _ADAPT_TARGET:
        TARGET = min(55000, max(2500, int(round(max(span_x, span_y) * umpx / 0.25))))  # ~0.25um/px, capped
